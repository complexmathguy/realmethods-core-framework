...
public class SHA1
{
    ...
    public void clear()
    {
        for(int nI = 0; nI < m_state.length; nI++)
            m_state[nI] = 0;

        m_lCount = 0L;
        for(int nI = 0; nI < m_digestBits.length; nI++)
            m_digestBits[nI] = 0;
    ...
    {
        return m_block[nI] = rol(m_block[nI], 24) & 0xff00ff00 | rol(m_block[nI], 8) & 0xff00ff;
    }

    final int blk(int nI)
    {
        return m_block[nI & 0xf] = rol(m_block[nI + 13 & 0xf] ^ m_block[nI + 8 & 0xf] ^ m_block[nI + 2 & 0xf] ^ m_block[nI & 0xf], 1);
    }
    ...
    {
        data[nZ] += ((data[nW] | data[nX]) & data[nY] | data[nW] & data[nX]) + blk(nI) + 0x8f1bbcdc + rol(data[nV], 5);
        data[nW] = rol(data[nW], 30);
    }
    ...
    public void update(byte bB)
    {
        ...
        m_lCount += 8L;
        m_block[m_nBlockIndex >> 2] &= ~(255 << nMask);
    ...
    public void update(String sData)
    {
        for(int nI = 0; nI < sData.length(); nI++)
            update((byte)(sData.charAt(nI) & 0xff));
    ...
    public void finalize()
    {
        ...
        for(int nI = 0; nI < 8; nI++)
            bits[nI] = (byte)(int)(m_lCount >>> (7 - nI << 3) & 255L);

        update((byte)-128);
        while(m_nBlockIndex != 56) 
            update((byte)0);
        for(int nI = 0; nI < bits.length; nI++)
            update(bits[nI]);
    ...
    public byte[] getDigest()
    {
        ...
        System.arraycopy(m_digestBits, 0, result, 0, 20);
        return result;
    }
    ...
    public String toString()
    {
        ...
        for(int nI = 0; nI < 20; nI++)
        {
            buf.append("0123456789abcdef".charAt(m_digestBits[nI] >>> 4 & 0xf));
            buf.append("0123456789abcdef".charAt(m_digestBits[nI] & 0xf));
        }
    ...
    public boolean selfTest()
    {
        ...
        tester.update("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq");
        tester.finalize();
    ...
    private static final byte SELFTEST_DIGEST[] = {
        -124, -104, 62, 68, 28, 59, -46, 110, -70, -82, 
        74, -95, -7, 81, 41, -27, -27, 70, 112, -15
    };
